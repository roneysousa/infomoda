object frmGerarFatura: TfrmGerarFatura
  Left = 164
  Top = 143
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Gerar Fatura'
  ClientHeight = 544
  ClientWidth = 800
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 800
    Height = 26
    Align = alTop
    Alignment = taLeftJustify
    Color = clBlue
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -19
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
  end
  object Panel1: TPanel
    Left = 0
    Top = 26
    Width = 800
    Height = 111
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object Label2: TLabel
      Left = 168
      Top = 37
      Width = 54
      Height = 13
      Caption = 'Data\Hora:'
      Transparent = True
    end
    object Label3: TLabel
      Left = 8
      Top = 63
      Width = 49
      Height = 13
      Caption = 'Vendedor:'
      Transparent = True
    end
    object Label4: TLabel
      Left = 8
      Top = 84
      Width = 35
      Height = 13
      Caption = 'Cliente:'
      Transparent = True
    end
    object btGravar: TSpeedButton
      Left = 518
      Top = 29
      Width = 82
      Height = 55
      Cursor = crHandPoint
      Hint = 'Gravar Pedido'
      Caption = '&Gravar Pedido'
      Flat = True
      Glyph.Data = {
        
        0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      Visible = False
    end
    object btCancelar: TSpeedButton
      Left = 607
      Top = 29
      Width = 82
      Height = 55
      Cursor = crHandPoint
      Hint = 'Cancelar'
      Caption = '&Cancelar Pedido'
      Flat = True
      Glyph.Data = {
        
        0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B0B}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      Visible = False
    end
    object spFechar: TSpeedButton
      Left = 703
      Top = 29
      Width = 82
      Height = 55
      Cursor = crHandPoint
      Hint = 'Fechar'
      Caption = '&Fechar Janela'
      Flat = True
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentShowHint = False
      ShowHint = True
      Visible = False
      OnClick = spFecharClick
    end
    object Label5: TLabel
      Left = 8
      Top = 37
      Width = 36
      Height = 13
      Caption = 'Pedido:'
    end
    object Label1: TLabel
      Left = 8
      Top = 13
      Width = 54
      Height = 13
      Caption = '&N.'#186' Pedido:'
    end
    object dbePedido: TEdit
      Left = 64
      Top = 32
      Width = 100
      Height = 21
      MaxLength = 6
      TabOrder = 1
      OnChange = dbePedidoChange
      OnEnter = dbePedidoEnter
      OnKeyPress = dbePedidoKeyPress
    end
    object dbeDTPEDI: TDateEdit
      Left = 224
      Top = 32
      Width = 121
      Height = 21
      NumGlyphs = 2
      ReadOnly = True
      TabOrder = 2
    end
    object dbeHOPEDI: TEdit
      Left = 352
      Top = 31
      Width = 71
      Height = 21
      ReadOnly = True
      TabOrder = 3
    end
    object cmbNMVEND: TEdit
      Left = 64
      Top = 55
      Width = 441
      Height = 21
      ReadOnly = True
      TabOrder = 4
    end
    object cmbNMCLIE: TEdit
      Left = 64
      Top = 78
      Width = 441
      Height = 21
      ReadOnly = True
      TabOrder = 5
    end
    object edtNRPEDI: TEdit
      Left = 64
      Top = 8
      Width = 100
      Height = 21
      MaxLength = 7
      TabOrder = 0
      OnChange = edtNRPEDIChange
      OnEnter = edtNRPEDIEnter
      OnKeyPress = edtNRPEDIKeyPress
    end
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 503
    Width = 800
    Height = 41
    Align = alBottom
    TabOrder = 2
    object btFatura: TBitBtn
      Left = 600
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Faturar...'
      TabOrder = 0
      OnClick = btFaturaClick
      Glyph.Data = {
        
        2207000000000000000777777777777777777777777777777777}
    end
    object btFechar: TBitBtn
      Left = 712
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Fecha&r'
      TabOrder = 1
      OnClick = btFecharClick
      Glyph.Data = {
        
        4B4B4E1E1FFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object pnlTOTAL: TPanel
    Left = 0
    Top = 449
    Width = 800
    Height = 54
    Align = alBottom
    TabOrder = 3
    object lbl_LEG_TOPEDI: TRxLabel
      Left = 440
      Top = 21
      Width = 121
      Height = 16
      Caption = 'TOTAL DO PEDIDO'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Panel3: TPanel
      Left = 568
      Top = 1
      Width = 231
      Height = 52
      Align = alRight
      TabOrder = 0
      object lbl_TOPEDI: TLabel
        Left = 172
        Top = 12
        Width = 53
        Height = 32
        Alignment = taRightJustify
        Caption = '0,00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -27
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
  end
  object pnlGrid: TPanel
    Left = 0
    Top = 137
    Width = 800
    Height = 312
    Align = alClient
    TabOrder = 4
    object GridItens: TRxDBGrid
      Left = 1
      Top = 1
      Width = 798
      Height = 310
      Align = alClient
      DataSource = dsItensPedidos
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = GridItensCellClick
      OnColEnter = GridItensColEnter
      OnDrawColumnCell = GridItensDrawColumnCell
      OnKeyDown = GridItensKeyDown
      OnKeyPress = GridItensKeyPress
      Columns = <
        item
          Alignment = taRightJustify
          Expanded = False
          FieldName = 'TFA_NRFATU'
          ReadOnly = True
          Title.Caption = 'FATURA'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taRightJustify
          Expanded = False
          FieldName = 'TFA_CDPROD'
          ReadOnly = True
          Title.Caption = 'C'#211'DIGO'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TFA_NMPROD'
          ReadOnly = True
          Title.Caption = 'DESCRI'#199#195'O'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'TFA_QTPROD'
          ReadOnly = True
          Title.Caption = 'QUANTIDADE'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TFA_VLUNIT'
          ReadOnly = True
          Title.Caption = 'VL.UNITARIO'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TFA_VLSUBT'
          ReadOnly = True
          Title.Caption = 'SUB-TOTAL'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'TFA_FLSITU'
          Title.Caption = 'SITUA'#199#195'O'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end>
    end
  end
  object qryPedido: TQuery
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      'Select * from "SACPED.DB" Where (PED_NRPEDI=:pCDPEDI)')
    Left = 328
    Top = 106
    ParamData = <
      item
        DataType = ftString
        Name = 'pCDPEDI'
        ParamType = ptInput
        Value = ''
      end>
  end
  object dsItensPedidos: TDataSource
    DataSet = tbItensPedidos
    Left = 400
    Top = 106
  end
  object qryCliente: TQuery
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      'Select CLI_CDCLIE, CLI_NMCLIE from "SACCLI.DBF" '
      'Where (CLI_CDCLIE=:pCDCLIE)')
    Left = 456
    Top = 106
    ParamData = <
      item
        DataType = ftString
        Name = 'pCDCLIE'
        ParamType = ptInput
        Value = ''
      end>
  end
  object qryVendedor: TQuery
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      'Select VEN_CDVEND, VEN_NMVEND from "SACVEN.DBF"'
      'Where (VEN_CDVEND=:pCDVEND)')
    Left = 488
    Top = 106
    ParamData = <
      item
        DataType = ftString
        Name = 'pCDVEND'
        ParamType = ptInput
        Value = ''
      end>
  end
  object qryItensPedido: TQuery
    OnCalcFields = qryItensPedidoCalcFields
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      'Select * from SACITP  Where (ITP_NRPEDI = :pNRPEDI) '
      'and (ITP_FLSITU="A")')
    Left = 256
    Top = 106
    ParamData = <
      item
        DataType = ftString
        Name = 'pNRPEDI'
        ParamType = ptInput
        Value = ''
      end>
    object qryItensPedidoITP_NRPEDI: TStringField
      FieldName = 'ITP_NRPEDI'
      Origin = 'INFOMODA."SACITP.DB".ITP_NRPEDI'
      Size = 7
    end
    object qryItensPedidoITP_NRITEM: TStringField
      FieldName = 'ITP_NRITEM'
      Origin = 'INFOMODA."SACITP.DB".ITP_NRITEM'
      Size = 3
    end
    object qryItensPedidoITP_CDPROD: TStringField
      FieldName = 'ITP_CDPROD'
      Origin = 'INFOMODA."SACITP.DB".ITP_CDPROD'
      Size = 13
    end
    object qryItensPedidoITP_TAMANH: TStringField
      FieldName = 'ITP_TAMANH'
      Origin = 'INFOMODA."SACITP.DB".ITP_TAMANH'
      Size = 2
    end
    object qryItensPedidoITP_QTPROD: TFloatField
      FieldName = 'ITP_QTPROD'
      Origin = 'INFOMODA."SACITP.DB".ITP_QTPROD'
    end
    object qryItensPedidoITP_VLUNIT: TCurrencyField
      FieldName = 'ITP_VLUNIT'
      Origin = 'INFOMODA."SACITP.DB".ITP_VLUNIT'
    end
    object qryItensPedidoITP_FLSITU: TStringField
      FieldName = 'ITP_FLSITU'
      Origin = 'INFOMODA."SACITP.DB".ITP_FLSITU'
      Size = 1
    end
    object qryItensPedidoITP_VLUNI2: TCurrencyField
      FieldKind = fkLookup
      FieldName = 'ITP_VLUNI2'
      LookupDataSet = qryProdutos
      LookupKeyFields = 'PRO_CDBARR'
      LookupResultField = 'PRO_VLVEND'
      KeyFields = 'ITP_CDPROD'
      Lookup = True
    end
    object qryItensPedidoITE_SUBTOT: TCurrencyField
      FieldKind = fkCalculated
      FieldName = 'ITE_SUBTOT'
      Calculated = True
    end
    object qryItensPedidoITP_NMPROD: TStringField
      FieldKind = fkLookup
      FieldName = 'ITP_NMPROD'
      LookupDataSet = qryProdutos
      LookupKeyFields = 'PRO_CDBARR'
      LookupResultField = 'PRO_NMPROD'
      KeyFields = 'ITP_CDPROD'
      Size = 40
      Lookup = True
    end
    object qryItensPedidoITP_NRFATU: TStringField
      FieldName = 'ITP_NRFATU'
      Origin = 'INFOMODA."SACITP.DB".ITP_NRFATU'
      Size = 10
    end
  end
  object qryProdutos: TQuery
    Active = True
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      
        'Select PRO_CDPROD, PRO_CDBARR, PRO_NMPROD, PRO_VLVEND from SACPR' +
        'O')
    Left = 216
    Top = 106
    object qryProdutosPRO_CDPROD: TStringField
      FieldName = 'PRO_CDPROD'
      Origin = 'INFOMODA."SACPRO.DBF".PRO_CDPROD'
      Size = 13
    end
    object qryProdutosPRO_CDBARR: TStringField
      FieldName = 'PRO_CDBARR'
      Origin = 'INFOMODA."SACPRO.DBF".PRO_CDBARR'
      Size = 13
    end
    object qryProdutosPRO_NMPROD: TStringField
      FieldName = 'PRO_NMPROD'
      Origin = 'INFOMODA."SACPRO.DBF".PRO_NMPROD'
      Size = 40
    end
    object qryProdutosPRO_VLVEND: TFloatField
      FieldName = 'PRO_VLVEND'
      Origin = 'INFOMODA."SACPRO.DBF".PRO_VLVEND'
    end
  end
  object ImageList1: TImageList
    Left = 432
    Top = 26
    Bitmap = {
      
      000000000000}
  end
  object tbItensPedidos: TTable
    AfterPost = tbItensPedidosAfterPost
    OnCalcFields = tbItensPedidosCalcFields
    DatabaseName = 'InfoModa'
    TableName = 'SACTFA.DB'
    Left = 448
    Top = 58
    object tbItensPedidosTFA_NRPEDI: TStringField
      FieldName = 'TFA_NRPEDI'
      Size = 7
    end
    object tbItensPedidosTFA_NRITEM: TStringField
      FieldName = 'TFA_NRITEM'
      Size = 3
    end
    object tbItensPedidosTFA_CDPROD: TStringField
      FieldName = 'TFA_CDPROD'
      Size = 13
    end
    object tbItensPedidosTFA_NRFATU: TStringField
      FieldName = 'TFA_NRFATU'
      Size = 10
    end
    object tbItensPedidosTFA_QTPROD: TFloatField
      FieldName = 'TFA_QTPROD'
    end
    object tbItensPedidosTFA_VLUNIT: TCurrencyField
      FieldName = 'TFA_VLUNIT'
    end
    object tbItensPedidosTFA_FLSITU: TBooleanField
      FieldName = 'TFA_FLSITU'
    end
    object tbItensPedidosTFA_NMPROD: TStringField
      FieldKind = fkLookup
      FieldName = 'TFA_NMPROD'
      LookupDataSet = qryProdutos
      LookupKeyFields = 'PRO_CDBARR'
      LookupResultField = 'PRO_NMPROD'
      KeyFields = 'TFA_CDPROD'
      Size = 40
      Lookup = True
    end
    object tbItensPedidosTFA_VLSUBT: TCurrencyField
      FieldKind = fkCalculated
      FieldName = 'TFA_VLSUBT'
      Calculated = True
    end
  end
  object qryDados: TQuery
    DatabaseName = 'InfoModa'
    SQL.Strings = (
      'Select * from SACTFA')
    Left = 344
    Top = 34
  end
end
